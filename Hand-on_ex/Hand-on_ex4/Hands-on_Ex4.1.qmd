...
There are several ways to plot ridgeline plot with R. This section uses the [ggridges](https://wilkelab.org/ggridges/index.html) package to plot ridgeline plots.
...
Note: The argument `calc_ecdf = TRUE` is required in `stat_density_ridges()`.
...
This section creates a raincloud plot to visualise the distribution of English score by race using **ggdist** and ggplot2.

### Plotting a Half Eye graph

First, a Half-Eye graph is plotted using [`stat_halfeye()`](https://mjskay.github.io/ggdist/reference/stat_halfeye.html) of **ggdist** package.
...
Next, the second geometry layer is added using [`geom_boxplot()`]() of ggplot2. This produces a narrow boxplot. We reduce the width and adjust the opacity.
...
Lastly, [`coord_flip()`](https://ggplot2.tidyverse.org/reference/coord_flip.html) of ggplot2 is used to flip the raincloud chart horizontally to give it the raincloud appearance. At the same time, `theme_economist()` of ggthemes package is used to give the raincloud chart a professional publishing standard look.
